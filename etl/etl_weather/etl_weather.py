...
def weather_api_to_df():
    ...
    for point in routemodel:
        if point.geopy_elapsed_dist_m >= WEATHER_RANGE * idx:
            idx += 1
            print("Making {}th weather call at ".format(idx), point.lat, point.lon, "at distance", point.geopy_elapsed_dist_m)
...
def seed_from_csv(filename):
    ...
    if engine.has_table(engine, Weather.__tablename__) and not db_session.query(Weather).count() == 0:
...
if __name__ == "__main__":
    weather_csv_filepath = normpath(abspath(join(dirname(__file__), '..', 'data', 'weather.csv'))) 
